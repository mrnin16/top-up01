...
export function CheckoutClient({ orderId }: Props) {
    ...
    <div className="mt-4 p-4 rounded-2xl border border-dashed" style={{ background: 'var(--surface-2)', borderColor: 'var(--line)' }}>
      <div className="flex flex-col md:flex-row gap-5 items-center">
        ...
        <div className="text-center md:text-left">
          ...
          <button
            disabled={scanning}
            className={`px-4 py-2 rounded-xl border text-[12.5px] font-medium transition-all${scanning?' btn-busy':''}`}
  ...
  const bankPanel = () => (
    <div className="mt-4 p-4 rounded-2xl border border-dashed" style={{ background: 'var(--surface-2)', borderColor: 'var(--line)' }}>
      <h4 className="font-sora font-bold text-[15px] mb-3">{t('pickYourBank')}</h4>
    ...
    <div className="mt-4 p-4 rounded-2xl border border-dashed" style={{ background: 'var(--surface-2)', borderColor: 'var(--line)' }}>
      ...
      <div className="grid grid-cols-2 gap-2.5">
        [
          ...
          { label: t('expiry'),     col: '',             key: 'exp',  type: 'text',     ph: 'MM/YY', mono: true,
            change: (v: string) => { let n = v.replace(/\D/g,'').slice(0,4); if(n.length>2) n=n.slice(0,2)+'/'+n.slice(2); setCard({...card, exp: n}); } },
          { label: t('cvc'),        col: '',             key: 'cvc',  type: 'text',     ph: '123', mono: true,
  ...
  const payBtn = (full = false) => (
    <button disabled={!canPay || scanning || bankMut.isPending}
      ...
      {method === 'khqr' ? t('iVePaidKhqr')
       : method === 'bank' ? `${t('continueToBank')} ${BANKS.find(b => b.id === bank)?.name}`
      ...
      <div
        ...
        <div
          ...
          <div className="flex items-center justify-between px-5 pt-3 pb-3 flex-none border-b" style={{ borderColor: 'var(--line)' }}>
            <div>
              <h2 className="font-sora font-bold text-[17px] m-0">{t('choosePayment')}</h2>
              ...
              <div>
                {order.discountCents > 0 && (
                  <p className="text-[11.5px] m-0 mb-0.5" style={{ color: 'var(--success)' }}>
                    {t('youSave')} ${(order.discountCents / 100).toFixed(2)}
                  </p>
                )}
                <div className="flex items-baseline gap-1.5">
                  <span className="font-sora font-bold text-[22px]">${total.toFixed(2)}</span>
                  <span className="text-[11px]" style={{ color: 'var(--muted)' }}>{t('including')} ${fee.toFixed(2)} {t('fee')} · USD</span>
                </div>
              </div>
              <div className="text-[11px] text-right" style={{ color: 'var(--muted)' }}>
                🛡 {t('sslPciBadge')}<br />{t('moneyBackGuarantee')}
              </div>
            </div>
            {payBtn(true)}
          </div>

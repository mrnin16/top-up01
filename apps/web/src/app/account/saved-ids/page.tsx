  ...
  if (!user) {
    return (
      <div className="min-h-screen grid place-items-center" style={{ background: 'var(--bg)' }}>
        <p style={{ color: 'var(--muted)' }}>Please <Link href="/auth/login" style={{ color: 'var(--brand)' }}>sign in</Link>.</p>
      </div>
    );
  }
    ...
    <div className="min-h-screen" style={{ background: 'var(--bg)' }}>
      ...
      <div className="max-w-2xl mx-auto px-6 py-8 route-content">
        <div className="flex items-center gap-3 mb-6">
          ...
          <h1 className="font-sora font-bold text-xl m-0">Saved Game IDs</h1>
        </div>

        <div className="text-center py-16 rounded-[22px] border"
          style={{ background: 'var(--surface)', borderColor: 'var(--line)' }}>
          <span className="text-5xl block mb-4">🎮</span>
          <h3 className="font-sora font-bold text-lg m-0 mb-2">No saved IDs yet</h3>
          <p className="text-sm mb-4 max-w-xs mx-auto" style={{ color: 'var(--muted)' }}>
            Your Game IDs are saved automatically when you complete a direct top-up.
          </p>
          <Link href="/" className="inline-block px-4 py-2 rounded-xl text-sm font-semibold no-underline"
            style={{ background: 'var(--brand)', color: '#fff' }}>Top up a game</Link>
        </div>
      </div>
    </div>
  );
